...
def find_geoloc(POI):
    ...
    try:
        latitude = float(POI['isLocatedAt']['schema:geo']
                         ['schema:latitude']['@value'])
        longitude = float(POI['isLocatedAt']['schema:geo']
                          ['schema:longitude']['@value'])
        # in the same try as if one is missing,
        # the geoloc data is not interesting anymore
    except KeyError:
        return None, None
...
def find_geoloc_POI(POI, UUID_gen, node_label):
    latitude, longitude = None, None
    try:
        latitude = float(POI['isLocatedAt'][0]
                         ['schema:geo']['schema:latitude'])
        longitude = float(POI['isLocatedAt'][0]
        ...
        return {'latitude': None, 'longitude': None,
                'UUID_gen': UUID_gen, 'node_label': node_label}
    return {'latitude': latitude, 'longitude': longitude,
            'UUID_gen': UUID_gen, 'node_label': node_label}


def create_POI_into_neo4j(POI: dict,
                          UUID_gen: str,
        ...
        with GraphDatabase.driver(db_neo4j_connect['uri'],
            ...
            with driver.session() as session:
                for POI in POI_list:
                    ...
                    UUID_gen = POI['UUID_gen']
                    node_label = POI['node_label']
